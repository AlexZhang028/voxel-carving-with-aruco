...
public:
    ...
    MultiImage(std::vector<std::string> &fileNames, int erodeIter, int dilateIter, cv::Scalar hsv_min, cv::Scalar hsv_max, int black_min, int white_min, bool flipHSVMask) {
        ...
        getSilhouettes(erodeIter, dilateIter, hsv_min, hsv_max, black_min, white_min, flipHSVMask);
    }
...
//            cv::Mat rt_Inverse = rt_Matrix.inv();
    ...
    void getSilhouettes(int erodeIter, int dilateIter, cv::Scalar hsv_min, cv::Scalar hsv_max, int black_min, int white_min, bool flipHSVMask) {
        ...
        for (int i = 0; i < images.size(); i++) {
            ...
            cv::Mat bwMask;
            cv::inRange(hsv, cv::Scalar(0, white_min, black_min), cv::Scalar(180, 255, 255), bwMask);
